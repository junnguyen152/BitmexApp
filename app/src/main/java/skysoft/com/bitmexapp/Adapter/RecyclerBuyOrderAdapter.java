...
public class RecyclerBuyOrderAdapter extends RecyclerView.Adapter<RecyclerBuyOrderAdapter.ViewHolder> {

    private List<OrderBookData> item;
    private Context context ;
    private long total = 0;
    public long finaltotal;
    public static long buytotal;

    public RecyclerBuyOrderAdapter(Context context, List<OrderBookData> item ) {
        this.item = item;
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.buyorder_row, null);
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        for(int i = 0; i < item.size(); i++){
            buytotal += item.get(i).getSize();
        }

        total += item.get(position).getSize();
        String strTotal = String.valueOf(total);
        holder.symbol.setText(strTotal);
        holder.size.setText(size);
        holder.price.setText(price);

        finaltotal = buytotal + RecyclerBuyOrderAdapter.buytotal;
